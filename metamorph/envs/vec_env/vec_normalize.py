...
class VecNormalize(VecEnvWrapper):
    ...
    def _obfilt_helper(self, obs, obs_type, update=True):
        ...
        try:
            ...
            batch_size = obs_p.shape[0]
            obs_p = obs_p.reshape(batch_size, cfg.MODEL.MAX_LIMBS, -1)
            mean = self.ob_rms[obs_type].mean.reshape(cfg.MODEL.MAX_LIMBS, -1)
            var = self.ob_rms[obs_type].var.reshape(cfg.MODEL.MAX_LIMBS, -1)
            obs_p[:, :, DEFAULT_OBS_DIM] = np.clip(
                (obs_p[:, :, DEFAULT_OBS_DIM] - mean)
                / np.sqrt(var + self.epsilon),
                -self.clipob,
                self.clipob,
            )
            obs_p[:, :, DEFAULT_CONTEXT_DIM] = obs['context'].reshape(batch_size, cfg.MODEL.MAX_LIMBS, -1)
            obs_p = obs_p.reshape(batch_size, -1)
